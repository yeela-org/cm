# -*- mode: yaml -*-
# This example configuration is no longer an example, it now configures gitStream for backbase-rnd.
# View the gitStream quickstart for actual examples: https://docs.gitstream.cm/examples/
manifest:
  version: 1.0
triggers:
  exclude:
...
automations:
  ...
  linearb_ai_review:
    ...
    run:
      - action: code-review@v1
        args:
          guidelines: |
            - Do not remind me about errors you have already reported
            - Do not comment "LGTM"
  ...
  label_missing_jira_info:
    if:
      - {{ not (has.jira_ticket_in_title or has.jira_ticket_in_desc or has.jira_ticket_in_branch) }}
      - {{ not pr.draft }}
      - {{ not is.bot }}
    run:
      - action: add-label@v1
